...
<script type="text/javascript">
	...
	function outOrder(id, name, obj) {

		if (confirm("确认菜品【" + name + "】已完成？")) {
			xmlAjaxRequest("outorder.order?orderId=" + id + "&dishesName="
					+ name + "&time=" + Math.random(), "get", true, null, deleteCallback, obj, null);
		}
	...
	function getOrderList(page) {
		xmlAjaxRequest("getorderbypage.order?page=" + page + "&time="
				+ Math.random(), "get", true, null, showList2, null, null);
	}

	function showList2(responseXml, obj) {
		...
		for ( var i = 0; i < orders.length; i++) {
			...
			var newLine = "<tr><td>" + orderId + "</td><td>" + tableId
					+ "</td><td>" + dishesName + "</td><td>"+ num + "</td>";
			newLine += "<td>";
			...
			newLine += "class='icon-remove-sign icon-large' title='出餐' onclick='outOrder("
					+ orderId + ",\"" + dishesName + "\",this)'></i>";
					...
					<div class="panel-body" style="padding-bottom: 10px">
						...
						<h4>
							<i class=" icon-envelope icon-large" style="color:orange;"></i>&nbsp;<span
								id="msg">${sessionScope.outInfo}</span>
						</h4>
...
<%--								${sessionScope.orderInfoKitchen}--%>
<%--								<c:forEach var="info" items="${sessionScope.orderInfoKitchen}">--%>
<%--									<tr hidden="1">--%>
<%--										${sessionScope.count=0}--%>
<%--									</tr>--%>

<%--								</c:forEach>--%>
<%--								<c:forEach var="info" items="${sessionScope.orderInfoKitchen}">--%>
<%--									<tr>--%>
<%--										<td>${sessionScope.count=((sessionScope.count)+1)}</td>--%>
<%--										<td>${info.tableId}</td>--%>
<%--										<td>${info.dishesName}</td>--%>
<%--										<td>${info.num}</td>--%>
<%--										<td><a href="outorder.order?orderId=${info.orderId}&dishesName=${info.dishesName}"><input type="button" style="background-color: red;border-radius: 25%"  value="出餐"></a></td>--%>
<%--									</tr>--%>

...
<script type="text/javascript">
	...
	function search() {
		...
		xmlAjaxRequest("getoperatedate.order?bt=" + begin.value + "&et="
				+ end.value +"&page=1&time=" + Math.random(), "get", true, null,
				showOrder, null, null);
	...
	function getOrderList(page){
		...
		var end = document.getElementById("et");
		if (isNaN(begin)){
			begin='2000-01-01 00:00';
		}
		if (isNaN(end)){
			end='2099-01-01 00:00';
		}
		xmlAjaxRequest("getoperatedate.order?bt=" + begin.value + "&et="
				+ end.value +"&page="+ page  + "&time=" + Math.random(), "get", true, null,showOrder, null, null);
	}
	function showOrder(responseXml, obj) {
		alert(responseXml)
		// var responseXml = createXmlDom(responseXML);
    ...
    function setEchartDetail(responseXml,obj){
		// alert(responseXml.valueOf())
		var ordersum = responseXml.getElementsByTagName("ordersum")
		var monthArry=['衬', '羊毛', '雪纺', '裤', '鞋', '袜子']
		var currentMonth = new Date().getMonth() + 1
		for (var i = 0;i< 6;i++){
			monthArry[i] = currentMonth--
		}
		if (isNaN(ordersum)){
			var orderData=[3005,10803,4444,5678,4980,6670]
		}else{
			var orderData= ordersum.nodeValue
		}
